...
""",
    ...
    assert [(node.name, node.data["containment"]) for node in reference_nodes] == [
        ("students", True)
    ]
    ...
    assert any(
        edge.type == "Contains"
        and edge.sourceId == classroom_id
        and edge.targetId == reference_id
        for edge in ir.edges
    )
    assert any(
        edge.type == "ReferenceType"
        and edge.sourceId == reference_id
        and edge.targetId == student_id
        for edge in ir.edges
    )
