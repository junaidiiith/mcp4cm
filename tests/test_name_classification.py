...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "placeholder-operation-template",
            "privateOperation",
            "Operation",
            "private operation",
            "operation",
        ...
        (
            "placeholder-attribute-template",
            "publicAttribute",
            "Property",
            "public attribute",
            "property",
        ...
        (
            "placeholder-type-letter-suffix",
            "ClassA",
            "Class",
            "class a",
            "class",
...
def test_record_level_domain_fixture_keeps_statistics_dummy_and_duplicates_aligned():
    ...
    labels = {label.element_id: label for label in extract_node_labels(record)}
    statistics_entries = {
        entry["name"]: (entry["classification"], entry["nameTokens"]) for entry in typed_name_entries(record)
    }
    dummy_nodes = {node.normalized_name: (node.classification, node.tokens) for node in derive_nodes(record)}
