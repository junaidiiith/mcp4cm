...
BERT_EVALUATION_THRESHOLD = 0.95
GNN_EVALUATION_THRESHOLD = 0.97
...
def run_bert_semantic(
    dataset: Dataset,
    progress: Callable[[dict[str, Any]], None] | None,
) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    pairs = bert_semantic_similarity_pairs(dataset, threshold=BERT_EVALUATION_THRESHOLD, progress=progress)
    return serialize_pair_groups(pairs)
...
def run_gnn(
    ...
    pairs = [
        SimilarPair(left_id, right_id, score, "gnn")
        for left_id, right_id, score in gnn_duplicate_pairs(
            dataset,
            threshold=GNN_EVALUATION_THRESHOLD,
            progress=progress,
        )
    ]
    return serialize_pair_groups(pairs)
...
DUPLICATE_TECHNIQUES = (
    ...
    TechniqueSpec(
        ...
        label="BERT Semantic",
        config={
            "similarityThreshold": BERT_EVALUATION_THRESHOLD,
            "libraryDefaultSimilarityThreshold": DEFAULT_SENTENCE_SIMILARITY_THRESHOLD,
            "modelName": "sentence-transformers/all-MiniLM-L6-v2",
            "batchSize": 8,
            "maxLength": 256,
            "semanticTextMode": "names_types_bag",
        },
        runner=run_bert_semantic,
    ),
    TechniqueSpec(
        id="gnn",
        label="Contrastive GNN",
        config={
            "similarityThreshold": GNN_EVALUATION_THRESHOLD,
            "libraryDefaultSimilarityThreshold": 0.85,
            "trainingConfig": asdict(GNNTrainingConfig()),
        },
        runner=run_gnn,
    ),
)

IGNORED_TECHNIQUES = ("graph_embedding", "graph_isomorphism")
...
def evaluate_dataset(*, dataset_name: str, evaluation_dir: Path) -> dict[str, Any]:
    ...
    result_payload = {
        ...
        "ignoredTechniques": list(IGNORED_TECHNIQUES),
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Examples:\n"
            "  python evaluation/duplicate_detection.py\n"
            "  python evaluation/duplicate_detection.py --only modelset-uml-xmi\n"
            "  python evaluation/duplicate_detection.py --evaluation-dir evaluation-runs\n"
        ),
    )
    parser.add_argument(
        "--only",
