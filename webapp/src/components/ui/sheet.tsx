...
function SheetOverlay({ className, ...props }: React.ComponentProps<typeof DialogPrimitive.Overlay>) {
  return (
    <DialogPrimitive.Overlay
      className={cn("fixed inset-0 z-50 bg-[color-mix(in_srgb,var(--foreground)_26%,transparent)]", className)}
      {...props}
    />
  );
}
...
const sheetVariants = cva(
  "fixed z-50 gap-4 border border-[var(--border)] bg-[var(--background)] p-6 shadow-[var(--shadow-strong)] transition ease-in-out",
  {
    variants: {
      side: {
